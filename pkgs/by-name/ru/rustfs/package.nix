...
rustPlatform.buildRustPackage (finalAttrs: {
  ...
  postPatch = ''
    ...
    # The Connect inventory store refuses directories with a setgid/group-write
    # bit (unix_directory_is_trusted, inventory.rs) when opening its in-tree
    # test fixtures. The build-user sandbox marks these dirs that way, so make
    # the /build source path owner-only to let the tests run.
    #chmod 700 /build/source /build/source/rustfs
    #chmod 700 /build 2>/dev/null || true
  '';
